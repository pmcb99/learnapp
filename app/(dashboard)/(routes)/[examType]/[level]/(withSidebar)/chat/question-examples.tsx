...
const QuestionExamples = ({
  ...
  askQuestionOnClick,
  variant,
}: QuestionExamplesProps) => {
  return (
    <div>
    {!variant && <div className="md:space-x-3 md:space-y-0 space-y-3 pt-4 grid md:grid-cols-3 md:grid-rows-1 grid-rows-3 grid-cols-1">
    ...
    {<div className="md:space-x-3 md:space-y-0 space-y-3 pt-4 grid md:grid-cols-3 md:grid-rows-1 grid-rows-3 grid-cols-1">
      {variant && questions.map((question) => (
        <Button
          className="h-auto w-auto border border-black dark:border-white group relative"
          onClick={() => askQuestionOnClick(question)}
          key={question}
          variant={variant}
        >
