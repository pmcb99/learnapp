...
import { LC_BUCKET_NAME } from "@/app/(dashboard)/(routes)/constants";


export default async function HomePage(
    params: { params: { subject: string, level: string, examType: string }; }
) {
    ...
    if (Array.isArray(examPaperKeys) && examPaperKeys.length > 0) {
        // Map over files.Contents and create a new array of objects conforming to CardProps type
        items.push(...examPaperKeys.map(file => ({
            label: `${file.Key?.split('/')[2]}${cleanUpFileName(file.Key?.split('/')[4] || '')}` || '',
            key: `${file.Key}`,
            href: `${hrefPrefix}/${file.Key?.split('/')[2]}/${file.Key?.split('/')[4].split('.')[0]}`,
            bgColor: "bg-blue-500",
            color: "text-blue-700"
        })));
    }
